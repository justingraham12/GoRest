...
func main() {
	...
	if err := GetEvents(allEvents); err != nil {
	...
	// Get my events
	if err = GetUsersEvents("Falkenfighter", usersEvents); err != nil {
		fmt.Println("Error getting Users events:", err)
		return
	}
...
func GetEvents(events *Events) (err error) {
	_, err = Client.Path("events").Get(events)
	return
}

// GET /users/:username/events
func GetUsersEvents(username string, events *Events) (err error) {
	_, err = Client.Path("users", username, "events").Get(events)
	return
}
